...
const AdminLeads = () => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (!roleData) { await supabase.auth.signOut(); router.replace("/admin/login"); return; }
      fetchLeads();
    };
    checkAuth();
  }, [router]);
  ...
  return (
    <>
      <SEOHead title="Leads | Looplic Admin" description="Admin leads" keywords="admin" noIndex />
      <div className="min-h-screen bg-background">
        <header className="sticky top-0 z-30 bg-primary text-primary-foreground px-4 py-3 flex items-center justify-between shadow-lg">
          <div className="flex items-center gap-2">
            <AppLink to="/admin/dashboard" className="p-1.5 rounded-full hover:bg-primary-foreground/10 transition-colors"><ArrowLeft size={16} /></AppLink>
            <Users size={20} />
              ...
              {leads.map((lead, i) => (
                <motion.div key={lead.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }} className="bg-card border border-border rounded-xl p-4 shadow-sm">
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-2.5">
                    {lead.project_type && <span className="text-[10px] bg-accent/10 text-accent px-2 py-0.5 rounded-full">{lead.project_type}</span>}
                    {lead.brand && <span className="text-[10px] bg-secondary/50 text-secondary-foreground px-2 py-0.5 rounded-full">{lead.brand}</span>}
                  </div>
                </motion.div>
              ))}
            </div>
